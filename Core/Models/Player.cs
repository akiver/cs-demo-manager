    ...
    public class Player : ObservableObject
    {
        ...
        public decimal KillDeathRatio
        {
            get
            {
                ...
                {
                    return Math.Round(_killCount / (decimal)_deathCount, 2);
                }
        ...
        [JsonProperty("suicide_count")] public int SuicideCount { get; set; }
        ...
        public double AverageHealthDamage
        {
            get
            {
                ...
                {
                    total = Math.Round(total / RoundPlayedCount, 1);
                }
        ...
        public decimal HeadshotPercent
        {
            get
            {
                ...
                if (_killCount > 0)
                {
                    headshotPercent = _headshotCount * 100 / (decimal)_killCount;
                }
        ...
        public double KillPerRound
        {
            get
            {
                if (RoundPlayedCount > 0)
                {
                    return Math.Round((double)KillCount / RoundPlayedCount, 2);
                }
        ...
        public double AssistPerRound
        {
            get
            {
                if (RoundPlayedCount > 0)
                {
                    return Math.Round((double)AssistCount / RoundPlayedCount, 2);
                }
        ...
        public double DeathPerRound
        {
            get
            {
                if (RoundPlayedCount > 0)
                {
                    return Math.Round((double)DeathCount / RoundPlayedCount, 2);
                }
        ...
        [JsonProperty("total_time_death")] public float TotalTimeDeath => TimeDeathRounds.Sum(kvp => kvp.Value);
        ...
        public double AverageTimeDeath
        {
            get
            {
                if (RoundPlayedCount > 0)
                {
                    return Math.Round((double)TotalTimeDeath / RoundPlayedCount, 2);
                }
        ...
        public void BackupFromPlayer(Player player)
        {
            ...
            TimeDeathRounds.Clear();
            foreach (KeyValuePair<int, float> kvp in player.TimeDeathRounds)
            {
                TimeDeathRounds.Add(kvp.Key, kvp.Value);
            }
