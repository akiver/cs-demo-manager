...
namespace Core.Models
{
    public class Demo : ObservableObject
    {
        ...
        public void BackupFromDemo(Demo demo)
        {
            ...
            foreach (MolotovFireStartedEvent e in demo.MolotovsFireStarted)
            {
                MolotovsFireStarted.Add(e);
            }

            IncendiariesFireStarted.Clear();
            foreach (MolotovFireStartedEvent e in demo.IncendiariesFireStarted)
            {
                IncendiariesFireStarted.Add(e);
            }
            ...
            Overtimes.Clear();
            foreach (Overtime o in demo.Overtimes)
            {
                Overtimes.Add(o);
            }
            ...
            DecoyStarted.Clear();
            foreach (DecoyStartedEvent e in demo.DecoyStarted)
            {
                DecoyStarted.Add(e);
            }

            BombPlanted.Clear();
            foreach (BombPlantedEvent e in demo.BombPlanted)
            {
                BombPlanted.Add(e);
            }

            BombDefused.Clear();
            foreach (BombDefusedEvent e in demo.BombDefused)
            {
                BombDefused.Add(e);
            }

            BombExploded.Clear();
            foreach (BombExplodedEvent e in demo.BombExploded)
            {
                BombExploded.Add(e);
            }
