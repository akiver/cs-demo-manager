    ...
    public class DemoListViewModel : BaseViewModel, IDisposable
    {
        ...
        public string SelectedFolder
        {
            ...
            set
            {
                ...
                if (Properties.Settings.Default.LimitStatsFolder)
                {
                    _cacheService.Filter.Folder = value;
                }

                if (_isMainWindowLoaded)
                {
                    DispatcherHelper.CheckBeginInvokeOnUI(
                        async () => { await LoadDemosHeader(); });
                }
            }
        }
            ...
            get
            {
                return _deleteDemosCommand
                       ?? (_deleteDemosCommand = new RelayCommand<ObservableCollection<Demo>>(
                           async demos =>
                           {
                               var delete = await _dialogService.ShowMessageAsync(Properties.Resources.DialogSendToRecycleBinConfimation,
                                   MessageDialogStyle.AffirmativeAndNegative);
                               if (delete == MessageDialogResult.Negative)
                               {
                                   return;
                               }
                               ...
                               {
                                   await _dialogService.ShowMessageAsync(string.Format(Properties.Resources.DialogDemosSentToRecycleBin, demos.Count),
            ...
            get
            {
                return _removeDemosFromCacheCommand
                       ?? (_removeDemosFromCacheCommand = new RelayCommand<ObservableCollection<Demo>>(
                           async demos =>
                           {
                               var delete = await _dialogService.ShowMessageAsync(Properties.Resources.DialogRemoveDemosFromCacheConfirmation,
                                   MessageDialogStyle.AffirmativeAndNegative);
                               if (delete == MessageDialogResult.Negative)
                               {
                                   return;
                               ...
                               {
                                   await _dialogService.ShowMessageAsync(string.Format(Properties.Resources.DialogDemosRemovedFromCache, demos.Count),
                                       MessageDialogStyle.Affirmative);
                               }
                       ...
                       ?? (_exportExcelCommand = new RelayCommand<ObservableCollection<Demo>>(
                           async demos =>
                           {
                               if (Properties.Settings.Default.SelectedStatsAccountSteamID != 0)
                               {
                                   ...
                                   var isExportFocusedOnPlayer =
                                       await _dialogService.ShowExportPlayerStatsAsync(settingsViewModel.SelectedStatsAccount.Name);
                ...
                return _goToTickCommand
                       ?? (_goToTickCommand = new RelayCommand<Demo>(
                           async demo =>
                           {
                               ...
                               var result = await _dialogService.ShowInputAsync(Properties.Resources.DialogGoToTick,
                                   Properties.Resources.DialogEnterTick);
            ...
            get
            {
                return _watchHighlightCommand
                       ?? (_watchHighlightCommand = new RelayCommand<Demo>(
                           async demo => { await ProcessWatchHighOrLow(demo); },
                           demo => demo != null));
            }
        }
            ...
            get
            {
                return _watchLowlightCommand
                       ?? (_watchLowlightCommand = new RelayCommand<Demo>(
                           async demo => { await ProcessWatchHighOrLow(demo, false); },
                           demo => demo != null));
            }
        }
            ...
            get
            {
                return _showMoreDemosCommand
                       ?? (_showMoreDemosCommand = new RelayCommand(
                           async () => { await PaginateDemos(Properties.Settings.Default.DemosListSize); },
                           () => !IsBusy));
            }
        }
                ...
                return _copyShareCodeCommand
                       ?? (_copyShareCodeCommand = new RelayCommand<Demo>(
                           async demo =>
                           {
                               ...
                               {
                                   await _dialogService.ShowErrorAsync(Properties.Resources.DialogDemoShareCodeUnavailable,
                                       MessageDialogStyle.Affirmative);
                                   return;
                               }
            ...
            get
            {
                return _showDialogThirdPartySelectionCommand
                       ?? (_showDialogThirdPartySelectionCommand = new RelayCommand(
                           async () => { await _dialogService.ShowCustomDialogAsync(_dialogThirdPartySelection); },
                ...
                return _downloadDemoFromShareCodeCommand
                       ?? (_downloadDemoFromShareCodeCommand = new RelayCommand(
                           async () =>
                           {
                               ...
                               string shareCode = await _dialogService.ShowInputAsync(Properties.Resources.DialogTitleDownloadDemoFromShareCode,
                                   Properties.Resources.DialogMessageDownloadDemoFromShareCode);
                       ...
                       ?? (_showThirdPartyDemoCommand = new RelayCommand<string>(
                           async thirdPartyName =>
                           {
                               ...
                               if (Properties.Settings.Default.AskThirdPartyConfirmation)
                               {
                                   ...
                                   if (send == MessageDialogResult.Negative)
                                   {
                                       return;
                                   }
        ...
        private async Task AnalyzeDemosAsync(bool checkExists = false)
        {
            ...
            finally
            {
                ...
                if (demosNotFound.Any())
                {
                    DispatcherHelper.CheckBeginInvokeOnUI(async () => await _dialogService.ShowDemosNotFoundAsync(demosNotFound));
                }

                if (demosFailed.Any())
                {
                    DispatcherHelper.CheckBeginInvokeOnUI(async () => await _dialogService.ShowDemosFailedAsync(demosFailed));
                }
        ...
        private async Task<bool> AnalyzeDemoAsync(Demo demo, CancellationToken token)
        {
            ...
            try
            {
                ...
                if (demo.Status == DemoStatus.NAME_DEMO_STATUS_ERROR)
                {
                    demo.Status = DemoStatus.NAME_DEMO_STATUS_NONE;
                }
        ...
        private async Task ProcessDemosDownloaded(bool isRecentMatches = true)
        {
            ...
            finally
            {
                ...
                {
                    await _dialogService.ShowMessageAsync(string.Format(Properties.Resources.DialogDemosHaveBeenDownloaded, demoDownloadedCount),
                        MessageDialogStyle.Affirmative);
                    await LoadDemosHeader();
                }
            }
        ...
        private async Task ProcessWatchHighOrLow(Demo demo, bool isHighlight = true)
        {
            ...
            try
            {
                if (isWatchValidated)
                {
                    StartWatchHighOrLow(demo, isHighlight);
                }
            }
            catch (Exception e)
            {
