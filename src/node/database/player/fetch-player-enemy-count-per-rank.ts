...
async function fetchMatchChecksumsWithPlayer(steamId: string, filters: FetchPlayerFilters) {
  let query = db
    .selectFrom('matches')
...
async function fetchPlayerEnemiesRankInMatches(steamId: string, filters: FetchPlayerFilters, checksums: string[]) {
  let query = db
    .selectFrom('players')
    .select(['rank'])
...
export async function fetchPlayerEnemyCountPerRank(
  ...
  const enemiesRank = await fetchPlayerEnemiesRankInMatches(steamId, filters, matchChecksumsWithPlayer);
