...
function buildStatsQuery(steamIds: string[], filters?: FetchPlayerFilters) {
...
function buildRoundsQuery(steamIds: string[], filters?: FetchPlayerFilters) {
  const { count } = db.fn;
  let query = db
    .selectFrom('rounds')
...
export async function fetchPlayersMapsStats(
  steamIds: string[],
  filters?: FetchPlayerFilters,
): Promise<PlayerMapsStats[]> {
