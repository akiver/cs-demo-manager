...
export async function initializeClusterIfNeeded(password: string) {
  ...
  // ! move() below refuses an existing destination, even an empty one. Failing here rather than
  // after initdb keeps the message actionable and doesn't waste a full initialization.
  if (await fs.pathExists(dataFolderPath)) {
    throw new EmbeddedPostgresInitFailed(
      `${dataFolderPath} already exists but holds no PostgreSQL cluster, refusing to overwrite it.`,
    );
  }
