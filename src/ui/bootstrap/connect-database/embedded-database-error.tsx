...
export function EmbeddedDatabaseError({ children, hasError }: Props) {
  ...
  return (
    <div className="m-auto flex database-page-max-width flex-col">
      <p>
        {hasError ? (
          // ! This screen is also shown after the user disconnected on purpose. Reporting a startup
          // failure there would be plainly false.
          <Trans>CS Demo Manager couldn't start its built-in database.</Trans>
        ) : (
          <Trans>CS Demo Manager is not connected to its built-in database.</Trans>
        )}
      </p>
      {children}
