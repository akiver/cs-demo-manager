...
export function Select<ValueType extends string | number = string>({
  ...
  return (
    <select
      id={id}
      className="h-30 appearance-none rounded-4 border border-gray-400 bg-gray-50 pl-12 outline-hidden focus:border-gray-900 enabled:hover:border-gray-900 disabled:bg-gray-400"
      disabled={isDisabled}
      value={value}
      onChange={(event) => {
