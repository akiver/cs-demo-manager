...
export function Row({ children }: { children: ReactNode }) {
  return (
    <div className="flex flex-1 scrollbar-stable items-center justify-between gap-x-8 overflow-x-auto rounded-4 border border-gray-300 bg-gray-75 px-16 py-8">
      {children}
    </div>
  );
}
...
export function RowLeft({ children }: { children: ReactNode }) {
  return <div className="flex flex-none items-center gap-x-8">{children}</div>;
}
