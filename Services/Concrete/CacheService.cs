...
namespace Services.Concrete
{
    public class CacheService : ICacheService
    {
        ...
        public async Task<List<Demo>> GetFilteredDemoListAsync()
        {
            ...
            try
            {
                ...
                foreach (DemoBasicData basicData in demoBasicDataList)
                {
                    ...
                    if (!string.IsNullOrEmpty(Filter.Folder) && Path.GetDirectoryName(basicData.Path) != Filter.Folder ||
                        !File.Exists(basicData.Path))
                    {
                        continue;
                    }

                    // don't add it if its not cached
                    if (!HasDemoInCache(basicData.Id))
                    {
                        continue;
                    }
        ...
        public async Task<bool> AddSuspectToCache(string suspectSteamCommunityId)
        {
            ...
            if (ids.Contains(suspectSteamCommunityId))
            {
                return false;
            }
            ...
            if (accountList.Any(account => account.SteamId == suspectSteamCommunityId))
            {
                return false;
            }
        ...
        {
            string pathSuspectsFileJson = GetFilePath(SUSPECT_FILENAME);
            if (!File.Exists(pathSuspectsFileJson))
            {
        ...
        public async Task<List<Account>> GetAccountListAsync()
        {
            ...
            if (!File.Exists(pathAccountsFileJson))
            {
        ...
        public async Task<bool> AddAccountAsync(Account newAccount)
        {
            ...
            // Check if already in the list
            if (accounts.Any(account => account.SteamId == newAccount.SteamId))
            {
                return false;
            }
        ...
        public Task<bool> RemoveDemo(string demoId)
        {
            ...
            if (File.Exists(weaponFiredFilePath))
            {
                File.Delete(weaponFiredFilePath);
            }

            if (File.Exists(playerBlindedFilePath))
            {
                File.Delete(playerBlindedFilePath);
            }
        ...
        public async Task<ObservableCollection<WeaponFireEvent>> GetDemoWeaponFiredAsync(Demo demo)
        {
            try
            {
                ...
                if (!File.Exists(pathDemoFileJson))
                {
                ...
                weaponFiredList = await Task.Factory.StartNew(() =>
                    JsonConvert.DeserializeObject<ObservableCollection<WeaponFireEvent>>(json, _settingsJson));
        ...
        public async Task<ObservableCollection<PlayerBlindedEvent>> GetDemoPlayerBlindedAsync(Demo demo)
        {
            ...
            try
            {
                ...
                playerBlindedList = await Task.Factory.StartNew(() =>
                    JsonConvert.DeserializeObject<ObservableCollection<PlayerBlindedEvent>>(json, _settingsJson));
            }
            catch (Exception e)
            {
                Logger.Instance.Log(e);
        ...
        public async Task<List<string>> GetPlayersWhitelist()
        {
            ...
            if (!File.Exists(pathWhitelistFileJson))
            {
                return new List<string>();
            }
        ...
        public async Task<List<RankInfo>> GetRankInfoListAsync()
        {
            ...
            if (rankAccountList == null)
            {
                rankAccountList = new List<RankInfo>();
            }
        ...
        public async Task<Rank> GetLastRankAsync(long steamId = 0)
        {
            ...
            if (rankAccountList.Any())
            {
                ...
                if (rankInfo != null)
                {
                    rank = AppSettings.RankList.FirstOrDefault(r => r.Number == rankInfo.Number);
                }
            }
