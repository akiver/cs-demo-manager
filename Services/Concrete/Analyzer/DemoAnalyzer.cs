...
namespace Services.Concrete.Analyzer
{
    public abstract class DemoAnalyzer
    {
        ...
        private const double AVERAGE_KPR = 0.679; // average kills per round
        private const double AVERAGE_SPR = 0.317; // average survived rounds per round
        private const double AVERAGE_RMK = 1.277; // average value calculated from rounds with multiple kills

        // HTLV2 rating variables https://flashed.gg/posts/reverse-engineering-hltv-rating/
        private const double HLTV2_KAST_MOD = 0.0073; // KAST modifier
        private const double HLTV2_KPR_MOD = 0.3591; // KPR modifier
        private const double HLTV2_DPR_MOD = -0.5329; // DPR modifier
        private const double HLTV2_IMPACT_MOD = 0.2372; // Impact modifier
        private const double HLTV2_IMPACT_KPR_MOD = 2.13; //Impact KPR modifier
        private const double HLTV2_IMPACT_APR_MOD = 0.42; //Impact AssistPerRound modifier
        private const double HLTV2_IMPACT_OFFSET_MOD = -0.41; //Impact base modifier
        private const double HLTV2_ADR_MOD = 0.0032; // ADR modifier
        private const double HLTV2_OFFSET_MOD = 0.1587; // HLTV2 base modifier
        ...
        {
            if (!IsMatchStarted || e.Player == null)
            {
                return;
            }
        ...
        {
            if (!IsMatchStarted || e.Shooter == null || e.Weapon == null)
            {
                return;
            }
            ...
            {
                IsFreezetime = false; // fix for demos that round_freeze_end sometimes not fired https://github.com/akiver/cs-demo-manager/issues/273
                ...
                foreach (DemoInfo.Player pl in Parser.PlayingParticipants)
                {
                    ...
                    if (player != null)
                    {
                        player.EquipementValueRounds[CurrentRound.Number] = pl.CurrentEquipmentValue;
                    }
                }

                CurrentRound.EquipementValueTeamCt =
                    Parser.Participants.Where(a => a.Team.ToSide() == Side.CounterTerrorist).Sum(a => a.CurrentEquipmentValue);
                CurrentRound.EquipementValueTeamT =
                    Parser.Participants.Where(a => a.Team.ToSide() == Side.Terrorist).Sum(a => a.CurrentEquipmentValue);
                ...
                else
                {
                    double diffPercent =
                        Math.Abs(Math.Round(
                            ((double)CurrentRound.EquipementValueTeamCt - CurrentRound.EquipementValueTeamT) /
        ...
        protected void HandleFireNadeStarted(object sender, FireEventArgs e)
        {
            ...
            if (!IsMatchStarted || e.ThrownBy != null && e.ThrownBy.SteamID == 0)
            {
                return;
            }
        ...
        {
            if (!AnalyzePlayersPosition || !IsMatchStarted)
            {
                return;
            }
        ...
        {
            if (!IsMatchStarted || e.ThrownBy == null)
            {
                return;
            }
        ...
        {
            if (!IsMatchStarted || e.ThrownBy == null || !PlayersFlashQueue.Any())
            {
                return;
            }
            ...
            if (PlayersFlashQueue.Any())
            {
                ...
                foreach (DemoInfo.Player player in Parser.PlayingParticipants)
                {
                    ...
                    if (pl != null)
                    {
                        pl.FlashDurationTemp = player.FlashDuration;
                    }
                }
        ...
        {
            if (!AnalyzePlayersPosition || !IsMatchStarted || e.ThrownBy == null)
            {
                return;
            }
        ...
        {
            if (!IsMatchStarted || IsFreezetime || e.Player == null || e.Player.SteamID == 0)
            {
                return;
            }
        ...
        {
            if (!IsMatchStarted || e.Player == null || e.Weapon == null)
            {
                return;
            }
            ...
            if (e.Attacker != null)
            {
                attacker = Demo.Players.FirstOrDefault(player => player.SteamId == e.Attacker.SteamID);
            }
        ...
        protected void CreateNewRound(bool force = false)
        {
            ...
            foreach (Player pl in Demo.Players)
            {
                ...
                if (!pl.RoundsMoneyEarned.ContainsKey(CurrentRound.Number))
                {
                    pl.RoundsMoneyEarned.Add(CurrentRound.Number, 0);
                }

                if (!pl.StartMoneyRounds.ContainsKey(CurrentRound.Number))
                {
                    pl.StartMoneyRounds.Add(CurrentRound.Number, 0);
                }

                if (!pl.EquipementValueRounds.ContainsKey(CurrentRound.Number))
                {
                    pl.EquipementValueRounds.Add(CurrentRound.Number, 0);
                }

                if (!pl.TimeDeathRounds.ContainsKey(CurrentRound.Number))
                {
                    pl.TimeDeathRounds.Add(CurrentRound.Number, 0);
                }
        ...
        private static double ComputeHltvOrgRating(int roundCount, int kills, int deaths, int[] nKills)
        {
            ...
            double roundsWithMultipleKillsRating =
                (nKills[0] + 4 * nKills[1] + 9 * nKills[2] + 16 * nKills[3] + 25 * nKills[4]) / (double)roundCount / AVERAGE_RMK;
        ...
        private static double ComputeHltv2OrgRating(Player player, Demo demo)
        {
            ...
            {
                if (player.Kills.Any(k => k.RoundNumber == round) || player.Assists.Any(a => a.RoundNumber == round) ||
                    player.Deaths.Any(d => d.RoundNumber == round && d.IsTradeKill) || !player.Deaths.Any(d => d.RoundNumber == round))
                {
                    KASTrounds++;
                }
            }
        ...
        {
            if (killEvent.KillerSteamId == 0 || IsFirstKillOccured)
            {
                return;
            }
        ...
        protected void UpdateTeamScore(RoundEndedEventArgs e)
        {
            ...
            if (!IsOvertime)
            {
                if (IsHalfMatch)
                {
                    winner.ScoreSecondHalf++;
                }
                else
                {
                    winner.ScoreFirstHalf++;
                }
            }
            else
            {
