...
namespace Services.Concrete
{
    public class AccountStatsService : IAccountStatsService
    {
        ...
        public async Task<OverallStats> GetGeneralAccountStatsAsync(List<Demo> demos)
        {
            ...
            if (SelectedStatsAccountSteamId == 0)
            {
                return stats;
            }
            ...
            await Task.Run(() =>
            {
                foreach (Demo demo in demos)
                {
                    ...
                    if (player != null)
                    {
                        ...
                        stats.KnifeKillCount += demo.Kills.Count(k =>
                            k.KillerSteamId == SelectedStatsAccountSteamId && k.Weapon.Element == EquipmentElement.Knife);
                ...
                if (stats.Kast > 0)
                {
                    stats.Kast = (float)Math.Round(stats.Kast/ stats.MatchCount, 2);
                }
            });
        ...
        public async Task<MapStats> GetMapStatsAsync(List<Demo> demos)
        {
            ...
            await Task.Run(() =>
            {
                if (demos.Any())
                {
                    List<Demo> demosPlayerList =
                        demos.Where(demo => demo.Players.FirstOrDefault(p => p.SteamId == SelectedStatsAccountSteamId) != null).ToList();
                    if (demosPlayerList.Any())
                    {
                        stats.Dust2WinCount = demosPlayerList.Count(d =>
                            d.MapName == "de_dust2" && (GetWinStatusCode(d, SelectedStatsAccountSteamId) == 1 ||
                                                        GetWinStatusCode(d, SelectedStatsAccountSteamId) == 2));
                        stats.Dust2LossCount = demosPlayerList.Count(d =>
                            d.MapName == "de_dust2" && (GetWinStatusCode(d, SelectedStatsAccountSteamId) == -1 ||
                                                        GetWinStatusCode(d, SelectedStatsAccountSteamId) == -2));
                        stats.Dust2DrawCount =
                            demosPlayerList.Count(d => d.MapName == "de_dust2" && GetWinStatusCode(d, SelectedStatsAccountSteamId) == 0);
                        ...
                        stats.MirageWinCount = demosPlayerList.Count(d =>
                            d.MapName == "de_mirage" && (GetWinStatusCode(d, SelectedStatsAccountSteamId) == 1 ||
                                                         GetWinStatusCode(d, SelectedStatsAccountSteamId) == 2));
                        stats.MirageLossCount = demosPlayerList.Count(d =>
                            d.MapName == "de_mirage" && (GetWinStatusCode(d, SelectedStatsAccountSteamId) == -1 ||
                                                         GetWinStatusCode(d, SelectedStatsAccountSteamId) == -2));
                        stats.MirageDrawCount =
                            demosPlayerList.Count(d => d.MapName == "de_mirage" && GetWinStatusCode(d, SelectedStatsAccountSteamId) == 0);
                        ...
                        stats.InfernoWinCount = demosPlayerList.Count(d =>
                            d.MapName == "de_inferno" && (GetWinStatusCode(d, SelectedStatsAccountSteamId) == 1 ||
                                                          GetWinStatusCode(d, SelectedStatsAccountSteamId) == 2));
                        stats.InfernoLossCount = demosPlayerList.Count(d =>
                            d.MapName == "de_inferno" && (GetWinStatusCode(d, SelectedStatsAccountSteamId) == -1 ||
                                                          GetWinStatusCode(d, SelectedStatsAccountSteamId) == -2));
                        stats.InfernoDrawCount =
                            demosPlayerList.Count(d => d.MapName == "de_inferno" && GetWinStatusCode(d, SelectedStatsAccountSteamId) == 0);
                        ...
                        stats.TrainWinCount = demosPlayerList.Count(d =>
                            d.MapName == "de_train" && (GetWinStatusCode(d, SelectedStatsAccountSteamId) == 1 ||
                                                        GetWinStatusCode(d, SelectedStatsAccountSteamId) == 2));
                        stats.TrainLossCount = demosPlayerList.Count(d =>
                            d.MapName == "de_train" && (GetWinStatusCode(d, SelectedStatsAccountSteamId) == -1 ||
                                                        GetWinStatusCode(d, SelectedStatsAccountSteamId) == -2));
                        stats.TrainDrawCount =
                            demosPlayerList.Count(d => d.MapName == "de_train" && GetWinStatusCode(d, SelectedStatsAccountSteamId) == 0);
                        ...
                        stats.OverpassWinCount = demosPlayerList.Count(d =>
                            d.MapName == "de_overpass" && (GetWinStatusCode(d, SelectedStatsAccountSteamId) == 1 ||
                                                           GetWinStatusCode(d, SelectedStatsAccountSteamId) == 2));
                        stats.OverpassLossCount = demosPlayerList.Count(d =>
                            d.MapName == "de_overpass" && (GetWinStatusCode(d, SelectedStatsAccountSteamId) == -1 ||
                                                           GetWinStatusCode(d, SelectedStatsAccountSteamId) == -2));
                        stats.OverpassDrawCount = demosPlayerList.Count(d =>
                            d.MapName == "de_overpass" && GetWinStatusCode(d, SelectedStatsAccountSteamId) == 0);
                        ...
                        stats.CacheWinCount = demosPlayerList.Count(d =>
                            d.MapName == "de_cache" && (GetWinStatusCode(d, SelectedStatsAccountSteamId) == 1 ||
                                                        GetWinStatusCode(d, SelectedStatsAccountSteamId) == 2));
                        stats.CacheLossCount = demosPlayerList.Count(d =>
                            d.MapName == "de_cache" && (GetWinStatusCode(d, SelectedStatsAccountSteamId) == -1 ||
                                                        GetWinStatusCode(d, SelectedStatsAccountSteamId) == -2));
                        stats.CacheDrawCount =
                            demosPlayerList.Count(d => d.MapName == "de_cache" && GetWinStatusCode(d, SelectedStatsAccountSteamId) == 0);
                        ...
                        stats.CobblestoneWinCount = demosPlayerList.Count(d =>
                            d.MapName == "de_cbble" && (GetWinStatusCode(d, SelectedStatsAccountSteamId) == 1 ||
                                                        GetWinStatusCode(d, SelectedStatsAccountSteamId) == 2));
                        stats.CobblestoneLossCount = demosPlayerList.Count(d =>
                            d.MapName == "de_cbble" && (GetWinStatusCode(d, SelectedStatsAccountSteamId) == -1 ||
                                                        GetWinStatusCode(d, SelectedStatsAccountSteamId) == -2));
                        stats.CobblestoneDrawCount =
                            demosPlayerList.Count(d => d.MapName == "de_cbble" && GetWinStatusCode(d, SelectedStatsAccountSteamId) == 0);
                        ...
                        stats.VertigoWinCount = demosPlayerList.Count(d =>
                            d.MapName == "de_vertigo" && (GetWinStatusCode(d, SelectedStatsAccountSteamId) == 1 ||
                                                          GetWinStatusCode(d, SelectedStatsAccountSteamId) == 2));
                        stats.VertigoLossCount = demosPlayerList.Count(d =>
                            d.MapName == "de_vertigo" && (GetWinStatusCode(d, SelectedStatsAccountSteamId) == -1 ||
                                                          GetWinStatusCode(d, SelectedStatsAccountSteamId) == -2));
                        stats.VertigoDrawCount =
                            demosPlayerList.Count(d => d.MapName == "de_vertigo" && GetWinStatusCode(d, SelectedStatsAccountSteamId) == 0);
                        ...
                        stats.NukeWinCount = demosPlayerList.Count(d =>
                            d.MapName == "de_nuke" && (GetWinStatusCode(d, SelectedStatsAccountSteamId) == 1 ||
                                                       GetWinStatusCode(d, SelectedStatsAccountSteamId) == 2));
                        stats.NukeLossCount = demosPlayerList.Count(d =>
                            d.MapName == "de_nuke" && (GetWinStatusCode(d, SelectedStatsAccountSteamId) == -1 ||
                                                       GetWinStatusCode(d, SelectedStatsAccountSteamId) == -2));
                        stats.NukeDrawCount =
                            demosPlayerList.Count(d => d.MapName == "de_nuke" && GetWinStatusCode(d, SelectedStatsAccountSteamId) == 0);
        ...
        public async Task<WeaponStats> GetWeaponStatsAsync(List<Demo> demos)
        {
            ...
            await Task.Run(() =>
            {
                List<Demo> demosPlayerList = demos.Where(demo => demo.Players.FirstOrDefault(p => p.SteamId == SelectedStatsAccountSteamId) != null)
                    .ToList();
                if (demosPlayerList.Any())
                {
                    foreach (Demo demo in demosPlayerList)
                    {
                        ...
                        stats.KillAk47Count +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.AK47);
                        ...
                        stats.KillM4A4Count +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.M4A4);
                        ...
                        stats.KillM4A1Count +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.M4A1);
                        ...
                        stats.KillAugCount +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.AUG);
                        stats.DeathAugCount +=
                            demo.Kills.Count(
                                k => k.KilledSteamId != 0 && k.KilledSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.AUG);
                        ...
                        stats.KillSg553Count +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.SG556);
                        ...
                        stats.KillFamasCount +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.Famas);
                        ...
                        stats.KillAwpCount +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.AWP);
                        stats.DeathAwpCount +=
                            demo.Kills.Count(
                                k => k.KilledSteamId != 0 && k.KilledSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.AWP);
                        ...
                        stats.KillScoutCount +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.Scout);
                        ...
                        stats.KillScar20Count +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.Scar20);
                        ...
                        stats.KillG3Sg1Count +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.G3SG1);
                        ...
                        stats.KillMp7Count +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.MP7);
                        stats.DeathMp7Count +=
                            demo.Kills.Count(
                                k => k.KilledSteamId != 0 && k.KilledSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.MP7);
                        ...
                        stats.KillMp5Count +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.MP5SD);
                        stats.DeathMp5Count +=
                            demo.Kills.Count(
                                k => k.KilledSteamId != 0 && k.KilledSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.MP5SD);
                        ...
                        stats.KillMp9Count +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.MP9);
                        stats.DeathMp9Count +=
                            demo.Kills.Count(
                                k => k.KilledSteamId != 0 && k.KilledSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.MP9);
                        ...
                        stats.KillP90Count +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.P90);
                        stats.DeathP90Count +=
                            demo.Kills.Count(
                                k => k.KilledSteamId != 0 && k.KilledSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.P90);
                        ...
                        stats.KillBizonCount +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.Bizon);
                        ...
                        stats.KillMac10Count +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.Mac10);
                        ...
                        stats.KillUmp45Count +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.UMP);
                        ...
                        stats.KillNovaCount +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.Nova);
                        ...
                        stats.KillXm1014Count +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.XM1014);
                        ...
                        stats.KillMag7Count +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.Swag7);
                        ...
                        stats.KillSawedOffCount +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.SawedOff);
                        ...
                        stats.KillM249Count +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.M249);
                        ...
                        stats.KillNegevCount +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.Negev);
                        ...
                        stats.KillGlockCount +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.Glock);
                        ...
                        stats.KillUspCount +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.USP);
                        stats.DeathUspCount +=
                            demo.Kills.Count(
                                k => k.KilledSteamId != 0 && k.KilledSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.USP);
                        ...
                        stats.KillP2000Count +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.P2000);
                        ...
                        stats.KillP250Count +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.P250);
                        ...
                        stats.KillCz75Count +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.CZ);
                        ...
                        stats.KillDeagleCount +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.Deagle);
                        ...
                        stats.KillDualEliteCount +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.DualBarettas);
                        ...
                        stats.KillTec9Count +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.Tec9);
                        ...
                        stats.KillFiveSevenCount +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.FiveSeven);
                        ...
                        stats.KillHeGrenadeCount +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.HE);
                        ...
                        stats.KillMolotovCount +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.Molotov);
                        ...
                        stats.KillIncendiaryCount +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.Incendiary);
                        ...
                        stats.KillTazerCount +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.Zeus);
                        ...
                        stats.KillKnifeCount +=
                            demo.Kills.Count(
                                k => k.KillerSteamId != 0 && k.KillerSteamId == SelectedStatsAccountSteamId &&
                                     k.Weapon.Element == EquipmentElement.Knife);
                ...
                if (demos.Any())
                {
                    ...
                    if (demosPlayerList.Any())
                    {
                        ...
                        foreach (Demo demo in demosPlayerList)
                        {
                            ...
                            if (GetWinStatusCode(demo, SelectedStatsAccountSteamId) == 1 ||
                                GetWinStatusCode(demo, SelectedStatsAccountSteamId) == 2)
                            {
                                winCount++;
                            }
                            ...
                            foreach (KillEvent e in demo.Kills)
                            {
                                ...
                                {
                                    if (!velocityStats.ContainsKey(e.Weapon.Type))
                                    {
                                        velocityStats.Add(e.Weapon.Type, 0);
                                    }
        ...
        {
            if (accountSteamId == 0 || demo.ScoreTeamCt == 0 && demo.ScoreTeamT == 0)
            {
                return -3;
            }
            ...
            {
                if (demo.ScoreTeamCt == demo.ScoreTeamT)
                {
                    return 0;
                }
