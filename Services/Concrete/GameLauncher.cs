...
namespace Services.Concrete
{
    public class GameLauncher : IDisposable
    {
        ...
        private const int
            NEXT_ACTION_DELAY =
                2; // Seconds before the playback fast forward to the next kill / action, Fade in / out and messages are displayed during this period
                    ...
                    switch (type)
                    {
                        case EquipmentElement.Smoke:
                            ...
                            if (e.Tick - lastTick > maxTickDelayCount)
                            {
                                ...
                                generated += string.Format(Properties.Resources.text_message_start, ++actionCount, startToTick - nextActionDelayCount,
                                    message);
        ...
        private void GenerateHighLowVdm(bool isHighlight, bool isFromPlayerPerspective = true)
        {
            ...
            foreach (KillEvent e in demo.Kills)
            {
                    ...
                    || isHighlight && e.KillerSteamId == _config.FocusPlayerSteamId)
                {
                    ...
                    generated += string.Format(Properties.Resources.text_message_start, ++actionCount,
                        startTick < nextActionDelayCount ? 0 : startTick, message);
                    generated += string.Format(Properties.Resources.screen_fade_start, ++actionCount,
                        startTick < nextActionDelayCount ? 0 : startTick);
                    generated += string.Format(Properties.Resources.skip_ahead, ++actionCount,
                        startTick < nextActionDelayCount ? 0 : startTick + nextActionDelayCount, e.Tick - tickDelayCount);
                    long specSteamId;
                    if (isHighlight)
                    {
                    ...
                    generated += string.Format(Properties.Resources.spec_player, ++actionCount,
                        startTick < nextActionDelayCount ? 0 : startTick + nextActionDelayCount, specSteamId);
                    lastTick = e.Tick;
                }
            }
