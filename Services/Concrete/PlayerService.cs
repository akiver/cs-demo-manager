            ...
            await Task.Factory.StartNew(() =>
            {
                foreach (Player player in demo.Players)
                {
                    if (!playerRoundStats.ContainsKey(player))
                    {
                        ...
                        if (!player.StartMoneyRounds.ContainsKey(round.Number))
                        {
                            player.StartMoneyRounds[round.Number] = 0;
                        }

                        if (!player.EquipementValueRounds.ContainsKey(round.Number))
                        {
                            player.EquipementValueRounds[round.Number] = 0;
                        }
                    ...
                    foreach (KillEvent e in round.Kills)
                    {
                        if (e.KillerSteamId == player.SteamId)
                        {
                            ...
                            if (e.KillerVelocityZ > 0)
                            {
                                playerRoundStats[player].JumpKillCount++;
                            }
                        }
                    }
                }
